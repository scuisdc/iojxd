...
int recursive(int t, int x) {
    printf("%d ", t);
    return recursive(t+1, rand());
}
...
int main(int argc, const char *argv[]) {
    ...
    printf("changing dir\n");
    chdir("/Users/secondatke/Documents/Documents/SCHOOL");
    printf("dir changed\n");
    FILE *fp = nullptr;
    printf("opening file\n");
    fp = fopen("2014_u_courses.md", "r");
    char t[256] = { 0 };
    printf("%d\n", system("ls"));
    // printf("reading file\n");
    // fread(t, 1, 255, fp);
    ...
    while (true) {
        qsort(array_tosort, ARRAY_SIZE, sizeof(int), cmp);
    }
    return 0;
}
...
static void handle_sigsys_o(int const nr) {
...
static void handle_sigsys_x(int const nr, siginfo_t * const info, void * const context) {
...
static void install_handler() __attribute__((constructor));	//	!!! compiler extension

void install_handler()
{
    printf("install_handler\n");
    struct sigaction sa;
    memset (&sa, 0, sizeof (sa));
    sa.sa_handler = handle_sigsys_o;
    sa.sa_sigaction = handle_sigsys_x;
    ...
    char *errbuf = nullptr;
    // printf("%d\n", sandbox_init(kSBXProfilePureComputation, SANDBOX_NAMED, &errbuf));
}
